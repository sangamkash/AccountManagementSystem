...
type IKafkaConsumer interface {
	ProcessMessage(ctx context.Context, msg models.TransactionMessage) error
}

func StartConsumer(service IKafkaConsumer, brokers, topic, group string) {
	go func() {
		...
		for {
			...
			if err := service.ProcessMessage(context.Background(), tmsg); err != nil {
				...
				// In production: retry or send to dead-letter queue_producer
			}
		}
	}()
}
